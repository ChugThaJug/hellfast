  ...
  function copyContent() {
    ...
    if (videoResult.output_format === "step_by_step") {
      if (videoResult.content.sections) {
        ...
        contentText = videoResult.content;
      }
      ...
      contentText = videoResult.content.text || JSON.stringify(videoResult.content);
    }
  ...
  {:else if videoResult}
    <div class="mb-8">
      ...
      <div class="flex justify-end mb-4">
        <Button variant="outline" on:click={copyContent}>
      ...
      <div class="bg-muted/30 border rounded-lg p-6">
        ...
        {:else}
          <div class="prose max-w-none">
            <p>{JSON.stringify(videoResult.content)}</p>
          </div>
        {/if}
      </div>
    </div>
